...
public int binarySearch(int catNumToFind)
{
    ...
    while (low <= high) {
        int avg = (low + high)/2;
        if (store[avg].getCatNum() == catNumToFind) {return store[avg].getInventory();}
        else if (store[avg].getCatNum() < catNumToFind) {low = avg + 1;}
        else if (store[avg].getCatNum() > catNumToFind) {high = avg - 1;}
    }
    return -1;    
}
...
{
    //complete this method    
    int avg = (nLow + nHigh)/2;
    if (store[avg].getCatNum() == catNumToFind) {return store[avg].getInventory();}
    if (nLow > nHigh) {return -1;}
    if (store[avg].getCatNum() > catNumToFind) {return binarySearch(catNumToFind, nLow, avg - 1);}
    if (store[avg].getCatNum() < catNumToFind) {return binarySearch(catNumToFind, avg + 1, nHigh);}
    return -1;
}
public void setup()
{
